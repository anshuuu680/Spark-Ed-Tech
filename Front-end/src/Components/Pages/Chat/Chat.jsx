...
const Chat = () => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/all-users`, { withCredentials: true }
        );
  ...
  const handleStartChat = async (user) => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/chat/start-chat/${user?._id}`,{},
  ...
  return (
    <div className="w-full h-screen flex flex-col md:flex-row md:p-4 gap-4 overflow-hidden">
      <div
        className={`w-full md:w-1/3 h-full flex flex-col shadow-md rounded-lg p-4 py-8 overflow-hidden ${
          hasContent ? "hidden md:flex" : ""
        }`}
      >
              ...
              return (
                <NavLink
                  ...
                  className={`flex justify-between items-center p-3 rounded-lg cursor-pointer group transition-all
                                        ${
                                          isActive
                                            ? "bg-blue-500 dark:bg-dark-inside-card"
                ...
                >
                  <div className="flex items-center gap-4">
                    <img
                      ...
                      src={chat.userDetails?.avatar}
                      alt="Profile"
                    />
                    <div className="flex flex-col">
                      <h1 className="font-semibold text-gray-900 dark:text-gray-100">
                        {chat.userDetails?.fullName}
                      </h1>
      ...
      {/* Chat Window */}
      <div
        className={`w-full md:w-2/3 h-screen flex flex-col shadow-md rounded-lg p-4 overflow-hidden ${
          hasContent ? "flex" : "hidden md:flex"
        }`}
      >
        <Outlet />
      </div>
